...
# Internal normalization controls — not pathogens.
SKIP_TARGETS = frozenset({"PMMoV", "BRSV", "pepper mild mottle virus"})
...
def _should_skip_target(target_key: str) -> bool:
    return target_key.strip().lower() in {name.lower() for name in SKIP_TARGETS}


def _display_name(target_key: str, display_names: dict[str, str] | None) -> str:
    if display_names and target_key in display_names:
        return display_names[target_key]
    return target_key
...
def discover_target_keys(
    ...
    *,
    ...
    """Collect all pathogen target keys present in the trend window."""
    ...
    for sample in samples:
        ...
        for target_key in targets:
            if _should_skip_target(str(target_key)):
                continue
            keys.add(str(target_key))
    return sorted(keys)
...
def parse_plant_series(
    ...
    *,
    ...
    target_keys = discover_target_keys(samples, window_days=window_days)
    if not target_keys and display_names:
        target_keys = sorted(display_names.keys())

    history_by_target: dict[str, list[dict[str, Any]]] = {key: [] for key in target_keys}
    ...
    for target_key in target_keys:
        ...
        pathogens.append(
            {
                "name": _display_name(target_key, display_names),
    ...
    *,
    ...
    for name, bucket in rollups.items():
        ...
        row = {
            ...
            "trend": "rising" if rising_states > 0 else "stable",
        }
