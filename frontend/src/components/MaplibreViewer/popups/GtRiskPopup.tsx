...
export function GtRiskPopup({
  ...
  return (
    <Popup
      ...
      maxWidth="380px"
    >
      <div className={`bg-black/95 border border-amber-700/50 rounded-lg overflow-hidden font-mono ${GT_TEXT.xs}`}>
        <div className="px-3 py-2 border-b border-amber-800/40 bg-amber-950/40 flex items-center gap-2">
          ...
          <span className={`text-amber-300 font-bold tracking-widest ${GT_TEXT.xs}`}>
            {t('gtRisk.popupTitle')}
          </span>
          <button
            type="button"
        ...
        <div className="p-3 flex flex-col gap-2 max-h-72 overflow-y-auto styled-scrollbar">
          ...
          <div className={`grid grid-cols-3 gap-2 ${GT_TEXT.micro}`}>
            <div>
          ...
          {resolvedWeatherNoise >= 0.25 && (
            <div className={`border-t border-amber-900/40 pt-2 ${GT_TEXT.micro} text-orange-300/90 leading-relaxed`}>
          ...
          <div className="border-t border-amber-900/40 pt-2">
            <div className={`${GT_TEXT.micro} tracking-widest text-amber-500/90 font-bold mb-1.5`}>
              {t('gtRisk.costlySignals')}
            </div>
            {loadingSignals ? (
              <div className={`${GT_TEXT.micro} text-amber-600/80`}>{t('gtRisk.loadingSignals')}</div>
            ) : consolidatedSignals.length > 0 ? (
              <div className="space-y-1.5">
                {consolidatedSignals.map((signal) => (
                  <div
                    key={signal.signalKey}
                    className={`border-l-2 border-amber-700/60 pl-2 ${GT_TEXT.micro} text-[var(--text-secondary)]`}
                  >
              ...
              <div className={`${GT_TEXT.micro} text-amber-600/75 leading-relaxed`}>
                {t('gtRisk.noSignals')}
              </div>
            )}
          </div>
