    ...
    <div className={shellClass}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className={`${GT_TEXT.xs} font-mono font-bold tracking-widest text-amber-400`}>
            {t('gtBacktest.title').toUpperCase()}
          </span>
          {showCollectingBadge && (
            <span className="text-[14px] font-mono px-1.5 py-0.5 tracking-wider border bg-amber-900/25 border-amber-700/40 text-amber-300">
              {t('gtBacktest.collecting')}
            </span>
          )}
          {layerEnabled && passBadge != null && (
            <span
              className={`text-[14px] font-mono px-1.5 py-0.5 tracking-wider border ${
                passBadge
      ...
      {!isMinimized && (
        <div className="px-3 py-2 max-h-[22rem] overflow-y-auto styled-scrollbar space-y-2">
          {!layerEnabled ? (
            <div className="text-[14px] font-mono tracking-wider text-amber-600/70 py-1">
            ...
            <>
              <div className="flex gap-1">
                {(['operational', 'benchmark'] as TabId[]).map((tab) => (
                  <button
                    ...
                    className={`text-[12px] font-mono tracking-widest px-2 py-0.5 border transition-colors ${
                      activeTab === tab
              ...
              {activeTab === 'benchmark' ? (
                benchmarkPending ? (
                  <div className="text-[14px] font-mono tracking-wider text-amber-500/80 py-1">
                  ...
                  <>
                    <div className="text-[12px] font-mono tracking-wider text-amber-600/60">
                      {t('gtBacktest.benchmarkNote')}
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div className="border border-amber-800/30 bg-amber-950/15 px-2 py-1.5">
                        <div className="text-[12px] font-mono tracking-widest text-amber-600/80">
                          {t('gtBacktest.accuracy')}
                        </div>
                        <div className={`${GT_TEXT.xs} font-mono font-bold text-amber-200`}>
                    ...
                    <div className="text-[12px] font-mono tracking-wider text-amber-600/70 leading-relaxed">
                    ...
                    <div className="flex flex-wrap gap-2 text-[12px] font-mono tracking-wider">
                    ...
                    <div className="flex items-center gap-1.5 text-[12px] font-mono tracking-wider text-amber-500/90">
                    ...
                    {failures.length > 0 && (
                      <div>
                        <button
                          ...
                          className="text-[12px] font-mono tracking-widest text-red-400 hover:text-red-300"
                        >
                        ...
                        {showFailures && (
                          <div className="mt-1 space-y-1">
                            {failures.map((row) => (
                              <div
                                key={row.case_id}
                                className="border border-red-800/30 bg-red-950/15 px-2 py-1 text-[12px] font-mono text-red-200/90"
                              >
                                {row.name} ({row.kind})
                              </div>
                            ))}
                ...
                <>
                  <div className="border border-amber-800/25 bg-amber-950/10 px-2 py-1.5 space-y-1">
                    <div className="text-[12px] font-mono tracking-widest text-amber-500/90">
                      {t('gtBacktest.microTitle').toUpperCase()}
                    </div>
                    {micro?.enabled ? (
                      <>
                        <div className="text-[12px] font-mono tracking-wider text-amber-600/75">
                        ...
                        {microRegions.length > 0 ? (
                          <div className="space-y-0.5">
                            {microRegions.map((row) => (
                              <div
                                key={row.region}
                                className="text-[12px] font-mono text-amber-200/85 flex items-center gap-1.5"
                              >
                                {row.ignition && (
                                  <span className="text-orange-400 border border-orange-700/40 px-1 text-[10px]">
                          ...
                          <div className="text-[12px] font-mono tracking-wider text-amber-600/65">
                  ...
                  <div className="flex items-center justify-between gap-2 pt-1">
                    ...
                    <button
                      ...
                      className="shrink-0 text-[12px] font-mono tracking-widest px-2 py-0.5 border border-amber-700/45 bg-amber-950/25 text-amber-300 transition-colors hover:bg-amber-900/35 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                  ...
                  {autoLabelNote && (
                    <div className="text-[12px] font-mono tracking-wider text-amber-500/85">
                      {autoLabelNote}
                    </div>
                  )}

                  {!rolling || rolling.weeks_stored === 0 ? (
                    <div className="text-[12px] font-mono tracking-wider text-amber-600/70 py-1">
                  ...
                  {latest && !latest.scorable && (
                    <div className="text-[12px] font-mono tracking-wider text-amber-500/80">
                  ...
                  {(rolling.accuracy_series?.length ?? 0) > 0 && (
                    <div>
                      <div className="text-[12px] font-mono tracking-widest text-amber-600/80 mb-1">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {rolling.accuracy_series.map((point) => (
                          <span
                            key={point.week_id}
                            className="text-[12px] font-mono border border-amber-800/30 bg-amber-950/20 px-1.5 py-0.5 text-amber-200/90"
