...
export default function GtTopAlertsStrip({
  ...
  return (
    <div className={shellClass}>
      ...
      {alerts.length === 0 ? (
        <div className={`px-2.5 py-2 ${GT_TEXT.xs} font-mono tracking-wider text-amber-600/70`}>
          {t('gtAlerts.empty')}
        </div>
      ) : (
          ...
          {alerts.map((alert) => (
            ...
            >
              <div className="flex items-center gap-1">
                <span className={`truncate ${GT_TEXT.xs} font-mono font-bold uppercase text-amber-100`}>
                  {alert.regionLabel}
                ...
                {alert.ignition && (
                  <span className="shrink-0 border border-orange-700/50 px-1 text-[10px] font-mono text-orange-300">
                    {t('gtAlerts.ignition')}
                  </span>
                )}
                <ChevronRight
              ...
              <div className={`${GT_TEXT.micro} font-mono tracking-wider text-amber-500/90`}>
                {t('gtAlerts.line')
      ...
      <div className="border-t border-amber-900/30 px-2.5 py-1 text-[10px] font-mono leading-relaxed text-amber-600/65">
        {t('gtAlerts.hint')}
      </div>
    </div>
  );
