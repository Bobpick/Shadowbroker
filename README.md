...
> **Join the private InfoNet swarm (sb-testnet-0):** Click **NODE** in the dashboard, or run `./scripts/operator/meshnode.sh` for a headless participant. No manual peer list — fleet defaults discover the seed and pull the signed manifest automatically. Set `MESH_INFONET_FLEET_JOIN=false` in `.env` for a private solo node.
